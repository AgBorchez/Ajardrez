...
bool MoveGen::is_square_attacked(const Board& board, int sq, Color attacker_color) {
    int r = SquareUtils::row(sq);
    int c = SquareUtils::col(sq);
...
std::vector<Move> MoveGen::get_pseudo_legal_moves(const Board& board, int from) {
    ...
    int r = SquareUtils::row(from);
    int c = SquareUtils::col(from);

    // Peones
    if (p.type == PieceType::PAWN) {
        int dir = (p.color == Color::WHITE) ? 1 : -1;
        int next_sq = from + dir * 8;
        if (SquareUtils::is_valid(next_sq) && board.get_piece(next_sq).type == PieceType::NONE) {
        ...
        for (int dc : {-1, 1}) {
            if (c + dc >= 0 && c + dc < 8) {
                ...
                if (SquareUtils::is_valid(cap_sq) && board.get_piece(cap_sq).color != Color::NONE && board.get_piece(cap_sq).color != p.color) {
