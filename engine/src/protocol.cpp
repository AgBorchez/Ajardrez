...
std::string Protocol::process_command(const std::string& line) {
    ...
    if (command == "PING") {
        ...
        std::string sq_str;
        iss >> sq_str;
        int sq = SquareUtils::from_string(sq_str);
        if (sq == -1) return "MOVES ";

        auto moves = MoveGen::get_legal_moves(board, sq);
        std::string res = "MOVES ";
        for (size_t i = 0; i < moves.size(); ++i) {
            res += SquareUtils::to_string(moves[i].from) + SquareUtils::to_string(moves[i].to);
        ...
        if (move_str.length() >= 4) {
            int from = SquareUtils::from_string(move_str.substr(0, 2));
            int to = SquareUtils::from_string(move_str.substr(2, 2));
        ...
        if (best.from != -1) {
            return "BESTMOVE " + SquareUtils::to_string(best.from) + SquareUtils::to_string(best.to);
        }
        return "NOMOVE";
    }
